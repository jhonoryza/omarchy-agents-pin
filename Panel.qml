...
Panel {
  ...
  // Pin ("on top"): while pinned, light-dismiss paths (outside click, Esc,
  // IPC hide) are ignored so the panel stays put. Explicit actions still
  // work: bar-icon toggle, the pin switch itself, launching an agent, and
  // switching to a sibling bar panel (the single-popout model is preserved).
  // Default comes from the widget's shell.json entry, e.g.
  // `omarchy bar set dell.agents pinned true --json`.
  property bool pinned: setting("pinned", false)
  property bool _allowClose: false

  function close() { if (pinned && !_allowClose) return; controller.hide() }
  ...
  function toggle() {
    if (!opened) { open() }
    else { _allowClose = true; controller.hide(); _allowClose = false }
  }
  ...
  // Local non-modal variant (see PinnedKeyboardPanel.qml): while pinned
  // the overlay lets clicks and keyboard focus reach the apps behind it.
  PinnedKeyboardPanel {
    id: panel
    anchorItem: button
    owner: root
    ...
    PanelKeyCatcher {
      ...
      onTextKey: function(t) {
        ...
        else if (t === "p" || t === "P") root.pinned = !root.pinned
      }
      ...
      Flickable {
        ...
        Column {
          ...
          Item {
            ...
            // Reached from the hero's trailingControl, whose `root` resolves
            // to PanelHero — not this panel — so pin state goes through here.
            property bool pinned: root.pinned
            function togglePin() { root.pinned = !root.pinned }
          ...
          PanelHero {
            ...
            // Pin button: on = panel stays put (no hide on outside
            // click / Esc). Shortcut: `p` while the panel has focus.
            trailingControl: Component {
              Button {
                text: heroWrap.pinned === true ? "Pinned" : "Pin"
                selected: heroWrap.pinned === true
                bordered: true
                foreground: hero.foreground
                fontFamily: hero.fontFamily
                fontSize: Style.font.bodySmall
                verticalPadding: Style.spacing.controlPaddingY
                tooltipText: heroWrap.pinned === true ? "Unpin panel" : "Pin panel on top"
                onClicked: heroWrap.togglePin()
              }
            }

            // Pin control temporarily removed for diagnostics.
