...
// NOTE (dell.agents): this is a copy of Omarchy's stock Ui/KeyboardPanel
// with one addition — `pinned`. While pinned the popup goes non-modal so
// the user can keep working in the apps behind it: the input mask shrinks
// to the card, outside-click dismissal (here and on the twin surfaces) is
// off, and layer-shell keyboard focus is released. Unpinning restores the
// stock modal behavior. If upstream KeyboardPanel changes, port them here.
PanelWindow {
  id: root
  ...
  //
  // Pinned deviation: None while pinned, so typing keeps going to the apps
  // behind the panel. Panel shortcuts need focus and sleep until unpinned.
  WlrLayershell.keyboardFocus: root.pinned ? WlrKeyboardFocus.None : (open
  ...
  //
  // Pinned deviation: the mask shrinks to the card, so the rest of the
  // (invisible) overlay is click-through to the apps behind it — the same
  // trick the notification toasts use (Region { item: ... }).
  ...
  mask: root.pinned ? cardMask : fullMask
  ...
  // were meant for the apps behind it — and while pinned, so outside
  // clicks reach the apps instead of dismissing.
  MouseArea {
    id: dismissArea
    anchors.fill: parent
  ...
  // and never while pinned, so other monitors stay interactive too.
  //
