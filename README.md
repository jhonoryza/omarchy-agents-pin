# My Agents (clone of Omarchy's `omarchy.agents` + pin)

Cloned with `omarchy plugin clone omarchy.agents`, then extended with a
**Pin** toggle. Everything below is upstream documentation, except:

- **Pin button** in the hero (or `p` while the panel has focus): while pinned,
  outside-click / Esc / IPC-hide are ignored so the panel stays put.
- **Non-modal while pinned** (`PinnedKeyboardPanel.qml`, a copy of the stock
  `Ui/KeyboardPanel` plus a `pinned` flag): clicks and keyboard focus reach
  the apps behind the panel; only the card itself stays interactive.
  Panel shortcuts sleep until unpinned. Unpinning restores stock behavior.
- Default: `omarchy bar set dell.agents pinned true --json`.
